...
% This live script shows how to implement a kind of data augmentation called 
% mix up [1] /sample paring [2].  
% 
...
% 
% $\tilde{x}$ = $\lambda x_i +{\left(1-\lambda \right)x}_j$   where xi,xj are 
% raw input vectors 
% 
% $\tilde{y}$ = $\lambda y_i +{\left(1-\lambda \right)y}_j$  where yi,yj are 
% one-hot label encodings
% 
% 
% 
% The classification was done using fine-tuining with a pre-trained network 
% ResNet-18. The Matlab official documents [3-5] were also refferred. 
% 
% In this demo, a dataset called Multi-class Weather Dataset for Image Classification 
% [6] was used. The images in the figure above were derived from the dataset [6]. 
% The ratio in the title of each subfigure represents the composition of the classes.  
% 
% [1] Zhang, Hongyi, Moustapha Cisse, Yann N. Dauphin, and David Lopez-Paz. 
% "mixup: Beyond empirical risk minimization." _arXiv preprint arXiv:1710.09412_ 
% (2017).
% 
% [2] Inoue, Hiroshi. "Data augmentation by pairing samples for images classification." 
% _arXiv preprint arXiv:1801.02929_ (2018).
% 
% This code with custom training loop was made based on the Matlab Official 
% Document located at
% 
% [3] Train Network Uing Custom Training Loop: <https://jp.mathworks.com/help/deeplearning/ug/train-network-using-custom-training-loop.html 
...
% https://jp.mathworks.com/help/deeplearning/examples/transfer-learning-using-alexnet.html>
% 
% [6] Multi-class Weather Dataset for Image Classification: <https://data.mendeley.com/datasets/4drtyfjtfy/1 
% https://data.mendeley.com/datasets/4drtyfjtfy/1>
%% Load Training Data 
% First of all, please down load the data from [6] and unzip into your current 
% directory. Confirm that the folder of dataset2 is in your current directory. 
% Use |imagedatastore| to list up images to use. The images were split into training 
% and validation datset. In your first time to run this code, the images in the 
% dataset2 folder are re-stored into a new folder. Please confirm the folder in 
% the dataset2 after running this section. 
% 
% 
...
% In this demo, we synthesize training images like alpha blending. Use |imfuse| 
% function and name-value input of 'blend' for alpha blending. Image synthesizing 
% like this will be done in the main section. 
...
% We use |imagedatastore| to store the images in the folder dataset2. If you 
% have not download ResNet-18, please down-load it from "add-on".
...
%% Define Network
% Define the network based on the pre-trained network which was loaded in the 
% prior section. 
% 
% Create a |dlnetwork| object from the layer graph.
% 
% If you run this demo with other pre-trained network, an error will be returned 
% because the layer name to refer should be changed at each pre-trained network. 
...
% You can check if your GPU is qualified for that. 
% 
% <https://developer.nvidia.com/cuda-gpus https://developer.nvidia.com/cuda-gpus>
% 
% Note that if your GPU is very brand-new, maybe your Matlab is not ready to 
% use the GPU with this demo.  
...
% To monitor the classification loss with validation data, the validation dataset 
% other than training and test dataset are prepared. For that purpose, |MixUpPreProc| 
% function defined at the end of this script is used. See the supporting function 
% for the detail. 
...
%% 
% * The reference [2] says "To synthesize a new image, we take the most naive 
% approach, just averaging the intensity of two images for each pixel". 
% * This demo shows how to mix up some images to synthesize one image. The number 
% can be specified by the variable |numMixUp|. 
% * If the number of numMixUp is three, three images were randomly picked from 
% the training dataset and the images were weighted and summed up. For example, 
% consider the weight of α, β, and γ where α + β + γ = 1. The synthesized image 
...
% on a test set with the true labels.
% 
% Convert the data to a |dlarray| object with dimension format |'SSCB'|. For 
% GPU prediction, also convert the data to |gpuArray|.
% 
...
%% 
% To classify images using a |dlnetwork| object, use the |predict| function 
% and find the classes with the highest scores.
...
%% 
% * Display the synthesized images and show how much percent a certain image 
% class is used
% * The result of |classes| in Line 116 shows the order of each class. 
% * For example, the result of classes is dog, cat, and bird, the order of the 
% dog class and bird class is 1 and 3, respectively.  
% * Regarding the composition ratio, if the composition ratio is shown as [0.4 
% 0 0.6], the synthesized images were composed of 40 % of dog image and 60 % of 
% bird image. 
